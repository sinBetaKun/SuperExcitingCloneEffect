...
namespace SuperExcitingCloneEffect.CloneController
{
    internal class ParamsOfCloneNode
    {
        ...
        public ParamsOfCloneNode(CloneBlock block, long length, long frame, int fps)
        {
            Appear = block.Appear;
        ...
        public bool Update(CloneBlock block, long length, long frame, int fps)
        {
            ...
            if (Appear != appear || BlendMode != blendMode || !Draw.Equals(draw) || Opacity != opacity || Rotate != rotate || Mirror != mirror
                || !Center.Equals(center) || KeepPlace != keepPlace || !ExpXY.Equals(expXY) || TagName != tagName || Parent != parent
